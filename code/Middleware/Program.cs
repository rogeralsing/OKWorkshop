...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        var headers = new MessageHeader(new Dictionary<string, string>
        {
            { "TraceID", Guid.NewGuid().ToString() },
            { "SpanID", Guid.NewGuid().ToString() }
        });
        var system = new ActorSystem();
        var root = new RootContext(system, headers, next => async (c, target, envelope) =>
        {
            var newEnvelope = envelope.WithHeader("TraceID", c.Headers.GetOrDefault("TraceID")).WithHeader("SpanID", Guid.NewGuid().ToString()).WithHeader("ParentSpanID", c.Headers.GetOrDefault("SpanID"));
        ...
        }).WithReceiverMiddleware(next => async (context, envelope) =>
        {
            ...
            {
                var newEnvelope = envelope.WithHeader("TraceID", envelope.Header.GetOrDefault("TraceID")).WithHeader("SpanID", Guid.NewGuid().ToString()).WithHeader("ParentSpanID", envelope.Header.GetOrDefault("SpanID"));
        ...
        {
            var newEnvelope = envelope.WithHeader("TraceID", context.Headers.GetOrDefault("TraceID")).WithHeader("SpanID", Guid.NewGuid().ToString()).WithHeader("ParentSpanID", context.Headers.GetOrDefault("SpanID"));
