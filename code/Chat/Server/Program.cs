...
internal static class Program
{
    ...
    {
        var config = BindToLocalhost(8000).WithProtoMessages(ChatReflection.Descriptor).WithRemoteDiagnostics(true);
    ...
    private static void SpawnServer()
    {
        ...
        context.SpawnNamed(Props.FromFunc(ctx =>
        {
            switch (ctx.Message)
            {
                case Connect connect:
                    ...
                    ctx.Send(connect.Sender, new Connected
                    {
                        Message = "Welcome!"
                    });
                    break;
